...
describe("app.tsx polling replacement", () => {
  it("app.tsx source does not contain setInterval for session polling", () => {
...
describe("session-item.tsx title display", () => {
  it("session-item.tsx source references session.title", () => {
    const src = readFileSync(new URL("./session-item.tsx", import.meta.url), "utf-8")
    expect(src).toContain("session.title")
  })
})

describe("session-item.tsx expand panel progress stream", () => {
  it("session-item.tsx source uses subscribeProgressStream when panel is expanded", () => {
    const src = readFileSync(new URL("./session-item.tsx", import.meta.url), "utf-8")
    expect(src).toContain("subscribeProgressStream")
  })

  it("session-item.tsx source renders messages from progress stream", () => {
    const src = readFileSync(new URL("./session-item.tsx", import.meta.url), "utf-8")
